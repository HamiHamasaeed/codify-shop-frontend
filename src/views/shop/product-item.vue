    ...
    <div class="flex flex-col lg:flex-row items-center w-full max-w-4xl">
      ...
      <div class="lg:ml-6 md:w-1/3 lg:w-2/3 text-left">
        ...
        <p
          ...
          :class="{
            'bg-green-200 text-green-800 max-w-20':
              product.status === 'available',
            'bg-red-200 text-red-800 max-w-28': product.status !== 'available',
          }"
        >
          {{ product.status }}
        </p>
        ...
        <div class="mt-3 text-center lg:text-left">
          <p v-if="product.discount" class="text-lg text-gray-500">
            Old Price: <span class="line-through">{{ product.price }} IQD</span>
          </p>
          <div
            class="bg-indigo-400 mt-2 max-w-[250px] rounded-lg text-white text-center mx-auto lg:mx-0"
          >
            <p v-if="product.discount" class="text-2xl font-semibold">
              Price: {{ product.discount }} IQD
            </p>
        ...
        .filter(
          (p) =>
            p.categories.some((category) =>
              product.value.categories.includes(category)
